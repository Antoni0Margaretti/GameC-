...
public class PlayerController : MonoBehaviour
{
    ...
    // Это позволяет сохранить горизонтальный импульс, не затираемый обновлением ввода.
    ...
    // --- Параметр мягкой коллизии (soft collision) со стеной.
    // При попадании в эту зону цепление за стену начинается даже без идеального контакта.
    public float softWallMargin = 0.2f;
    ...
    void Update()
    {
        ...
        // Если персонаж цепляется за стену, но стену больше не удаётся обнаружить, выключаем режим цепления.
        ...
        // Если персонаж не цепляется, не скользит, не приседает и не находится в блокировке после wall jump:
        if (!isSlidingOnWall && !isSliding && !isCrouching && !isWallJumping)
        {
            ...
            {
                // В воздухе: при наличии ввода горизонтальная скорость плавно интерполируется.
                // Если ввода нет, сохраняется ранее заданная скорость (например, импульс от wall jump).
        ...
        // а движение по оси X не перезаписывается (чтобы сохранить импульс).
        if (isSlidingOnWall)
        {
            if (moveInput > 0 && !facingRight)
                Flip();
        ...
        // --- Прыжок
        // Если нажата кнопка прыжка, и персонаж находится на земле, может прыгать или цепляться за стену.
        ...
        // Если персонаж либо точно касается стены, либо попадает в «мягкую» зону,
        // находится не на земле, есть горизонтальный ввод и прошло достаточно времени после предыдущего цепления.
        if ((collisionController.IsTouchingWall || collisionController.IsInSoftWallZone(softWallMargin))
            && !grounded &&
    ...
    private void StartWallHang()
    {
        if (!isSlidingOnWall)
        {
            ...
            // Если цепление инициируется через мягкую зону, корректируем положение персонажа.
            if (collisionController.IsInSoftWallZone(softWallMargin))
            {
                collisionController.SnapToWall(transform, softWallMargin);
            }
    ...
    // --- Блокировка горизонтального управления после wall jump,
    // чтобы импульс wall jump сохранялся и не затирался обычным обновлением ввода.
