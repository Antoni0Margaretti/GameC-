...
public class PlayerController : MonoBehaviour
{
    ...
    public int maxJumps = 2;              // Для логики (рассчитывается динамически)
    ...
    private float originalGravityScale;       // Исходное значение gravityScale, сохранённое в Start()
    ...
    void Start()
    {
        ...
        // Сохраняем исходные параметры хитбокса для восстановления.
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Jump") && (grounded || jumpCount < maxJumps || isSlidingOnWall))
        {
            if (isSlidingOnWall)
            {
                // Если игрок находится в цеплении и нажимает Jump:
                // Если ввод в горизонтали направлен к стене (то есть пытается прыгнуть "в ту же сторону"), выполняется обычный вертикальный прыжок.
                ...
                {
                    // Иначе выполняем wall jump: персонаж получает диагональный импульс,
                    // горизонтальная составляющая задается wallJumpHorizForce, вертикальная – wallJumpForce.
                    rb.linearVelocity = new Vector2((facingRight ? 1 : -1) * wallJumpHorizForce, wallJumpForce);
                    jumpCount = 0; // Обнуляем счётчик, чтобы после wall jump оставался один прыжок (если понадобится)
                }
                StopWallSlide();
            ...
            {
                // Прыжок с пола – обычный вертикальный прыжок.
                rb.linearVelocity = new Vector2(rb.linearVelocity.x, jumpForce);
                jumpCount++;
            }
        }
        ...
        // --- Инициирование цепления за стену
    ...
    private void StartWallHang()
    {
        if (!isSlidingOnWall)
        {
            ...
            // Устанавливаем значение гравитации для цепления из параметра.
    ...
    private void StopWallSlide()
    {
        ...
        // Восстанавливаем исходное значение гравитации, сохранённое в Start().
